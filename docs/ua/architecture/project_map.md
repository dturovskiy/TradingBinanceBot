...
| Портфельний ризик | Багаторівнева/grouped політика портфельного ризику, розділення reason/model та fail-safe containment |
| Моніторинг / спостережуваність | Стан здоров’я, метрики, телеметрія та звіти |
| Надійність / обробка відмов | Bounded failure handling, secret-safe diagnostics, fail-safe containment |
...
5. Біржа/API, ризик, збереження стану, спостережуваність і надійність залишаються окремими доменами відповідальності.

Це порядок відповідальності та безпеки, а не точна послідовність реалізації запуску.

Risk ownership є багаторівневим: семантика reason/model залишається відокремленою від grouped/portfolio-level координації, щоб ризикові контури могли локалізувати відмови та діяти fail-safe без прив’язки публічного контракту до production-порогів або поточної exposure.
...
- [Довідник сімейств модулів](module_reference.md)
