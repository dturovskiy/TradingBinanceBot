...
Доказова база має визначати роль і походження матеріалів джерела, рішення та результату, не змішуючи ці поняття. Provenance не слід виводити лише з назв файлів, timestamp або локальних каталогів.
...
Ідентичність кандидата/конфігурації та ідентичність набору даних концептуально різні й мають бути явно пов’язані, коли оцінювання залежить від обох. Якщо обов’язкові прив’язки кандидата, набору даних або доказової бази не збігаються, агрегація має завершуватися за принципом fail-closed, а не створювати змішаний результат з неоднозначною provenance.
...
Scanner/research outputs є лише evidence inputs. Їх створення ізольоване від order placement, а bounded і failure-isolated research work не має rollout authority. Відсутні або некоректні обов’язкові scanner/evidence bindings блокують promotion за принципом fail-closed, а не трактуються як дозвіл продовжувати.

## 9. Паритет execution / domain

Паритет execution/domain — це окремий рівень валідації, призначений для виявлення випадків, коли replay або дослідження обходять суттєву семантику runtime. Він охоплює семантику виконання, часу, ризику та стану, релевантну цільовому шляху; сприятливий результат дослідження все одно може бути заблокований, якщо цей рівень відсутній або неузгоджений.
